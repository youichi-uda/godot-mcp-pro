...
get_gridmap_info     → изучить GridMap: MeshLibrary, границы занятых ячеек, количество по элементам (фильтр ячеек по элементу/границам)
```
...
reload_open_scripts → перезагрузить открытые вкладки скриптов с диска после внешних правок (Godot 4.7+; вкладки с несохранёнными изменениями не трогаются)
close_script   → закрыть вкладку скрипта (Godot 4.7+; отказывает при несохранённых изменениях, если не указан discard_unsaved=true)
```
...
add_virtual_joystick → экранный сенсорный джойстик, нажимающий действия ввода (Godot 4.7+; для проверки мышью включите input_devices/pointing/emulate_touch_from_mouse)
```
...
Перед редактированием файлов, открытых в редакторе, или в конце задачи вызовите `get_unsaved_state`, чтобы увидеть несохранённое, а затем `save_all`, чтобы сохранить все открытые сцены и изменённые буферы скриптов. В Godot 4.5/4.6 списки несохранённого равны `null` (неизвестно, а не «всё сохранено»), и сохраняются только сцены; для ни разу не сохранённых сцен нужен `save_scene` с путём.
...
set_game_speed      → задать Engine.time_scale во время игры: 0.25 = замедление для capture_frames, 4 = ускорение долгих ожиданий
```
...
setup_ik_modifier               → добавить IK-модификатор (TwoBoneIK3D, CCDIK3D, FABRIK3D, …) к Skeleton3D (Godot 4.6+)
```
